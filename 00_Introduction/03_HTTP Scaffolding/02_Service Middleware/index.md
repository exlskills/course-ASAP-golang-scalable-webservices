### Mission-specific Middleware

Aside from the generic middleware covered in the "Generic Middleware" section, we almost always ending up need some that's more specific to the data that we're working with. For instance, we might need some middleware to secure a route for users only. This is what we'll implement in our project's custom middleware.

### Our Middlware

In our `middleware/middleware.go` file, we'll setup the following:
